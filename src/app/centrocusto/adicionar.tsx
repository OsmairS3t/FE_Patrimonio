...
type CentroCusto = z.infer<typeof centroCustoSchema>
...
export function Adicionar({ setIsClose }: Props) {
  ...
  async function handleSubmitCentroCusto(data: CentroCusto) {
    await api.post('centrocusto', data)
    alert('Centro de Custo incluído com sucesso!')
    reset()
    setIsClose(false)
  }
  ...
  return (
    <div>
      <div className="flex flex-row justify-between">
        <h2 className="text-2xl font-semibold">Incluir Centro de Custo</h2>
        <button
          onClick={handleClose}
      ...
      <form
        onSubmit={handleSubmit(handleSubmitCentroCusto)}
        className="flex flex-col gap-4 mt-4"
      >
        <div className="flex flex-col gap-2 w-full">
          <label htmlFor="descricao">Centro de custo:</label>
          <input
